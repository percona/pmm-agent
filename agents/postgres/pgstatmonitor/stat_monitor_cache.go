...
func (ssc *statMonitorCache) getStatMonitorExtended(ctx context.Context, q *reform.Querier, normalizedQuery bool) (current, cache map[time.Time]map[string]*pgStatMonitorExtended, err error) {
	...
	if (pgMonitorVersion == pgStatMonitorVersion09) ||
		(pgMonitorVersion >= pgStatMonitorVersion10PG12 && pgMonitorVersion <= pgStatMonitorVersion10PG14 && prerelease != "" && prerelease != "rc.2") {
		// only pg_stat_monitor 0.9.0, 1.0.0-beta-2, 1.0.0-rc.1  supports state_code. It tells what is the query's current state.
